...
static int test_encrypt_ecb(void)
{
    ...
    // My own data
    uint8_t newKey[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
    memcpy(key, newKey, sizeof(newKey));
    
    uint8_t newIn[] = {0x00, 0x10, 0x20, 0x30, 0x01, 0x11, 0x21, 0x31, 0x02, 0x12, 0x22, 0x32, 0x03, 0x13, 0x23, 0x33 };
    memcpy(in, newIn, sizeof(newIn));
    ...
    for (int i = 0; i < AES_keyExpSize; i++)
        printf("%08x\t", ctx.RoundKey[i]);
    printf("\n\n\n");

    for (int i = 0; i < AES_keyExpSize; i += 4)
    {
        uint32_t word = (ctx.RoundKey[i] << 24) | (ctx.RoundKey[i+1] << 16) | (ctx.RoundKey[i+2] << 8) | ctx.RoundKey[i+3];
        printf("Word %02i: %08x\n", i/4, word);
    }
