...
typedef struct {
    ...
    u32 ChannelOccupiedFlag[(AES_NUM_CHANNELS+31)/32]; // Rounding up
    int highestOccupiedChannel;
} AES;
...
void AES_startComputationCBC(AES* InstancePtr, u32 channel, int encrypt, u8* data, u8* outData, u32 size, u8 IV[16], AES_CallbackFn callbackFn, void* callbackRef);
...
s32 AES_ReserveNewChannel(AES* InstancePtr, u32* outChannel);
s32 AES_FreeChannel(AES* InstancePtr, u32 channel);
...
#define AES_mReadMemory(Address) 0
